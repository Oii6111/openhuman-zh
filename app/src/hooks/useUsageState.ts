...
interface UsageState {
   ...
   * Used to suppress the 小鹈鹕-included-budget banner / modal: when the
...
async function fetchUsageData(): Promise<{
  ...
  // 小鹈鹕's billing/usage APIs at all (#2020). Missing/failed AI settings
...
export function useUsageState(activeChatRole: 'chat' | 'reasoning' = 'chat'): UsageState {
  ...
  // local Ollama), the 小鹈鹕 included-budget cycle does not gate the
  ...
  // 小鹈鹕 the included-budget cycle does not gate the user, so the
