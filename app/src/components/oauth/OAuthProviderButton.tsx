...
const BACKEND_UNAVAILABLE_MESSAGE =
  '小鹈鹕 cloud sign-in is temporarily unavailable. Please try again in a few minutes.';
...
const SAFE_READINESS_STARTUP_MESSAGE_PREFIXES = [
  'Finish choosing how 小鹈鹕 runs',
  '小鹈鹕 could not reach its local runtime',
];
...
const getOAuthStartupFailureMessage = (provider: OAuthProviderConfig, error?: unknown): string => {
  ...
  if (provider.id === 'twitter') {
    return 'Twitter/X sign-in could not start. Check that the Twitter OAuth app callback URL, client ID/secret, and requested scopes match the 小鹈鹕 backend, then try again.';
  }
